      ...
      <div className="w-full max-w-md max-h-[92dvh] md:max-h-[85dvh] rounded-2xl flex flex-col overflow-hidden"
        ...
        {method === 'subscription' && (
          <div className="flex-1 overflow-y-auto px-4 pb-4 min-h-0">
            <div className="rounded-xl p-3 mb-3 text-[12px] leading-snug"
              style={{ background: 'var(--accent-light)', border: '1px solid var(--border)' }}>
              <b>Как подключить свою Pro/Max подписку:</b>
              <ol className="list-decimal list-inside mt-1.5 space-y-1">
                <li>На <b>своём Mac</b> (где Claude уже залогинен) открой Terminal и выполни:
                  <pre className="font-mono text-[10.5px] mt-1 p-1.5 rounded overflow-x-auto"
                    style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
cat ~/.claude/.credentials.json
</pre>
                </li>
                <li>Скопируй весь вывод (это JSON вида <code className="font-mono text-[10.5px]">{`{"claudeAiOauth":{…}}`}</code>).</li>
                <li>Вставь ниже → «Скопировать на <b>{deviceName}</b>».</li>
                <li>Claude на этом устройстве заработает через твою подписку, без оплаты API.</li>
              </ol>
              <div className="mt-2 pt-2 text-[11px]" style={{ color: 'var(--muted)', borderTop: '1px solid var(--border)' }}>
                ⚠ Токен обновляется автоматически (expiresAt каждые ~8 часов), но если надолго
                уехал в офлайн — можешь повторить копирование.
              </div>
            </div>

            <label className="block text-[11.5px] mb-1" style={{ color: 'var(--muted)' }}>
              Содержимое ~/.claude/.credentials.json
            </label>
            <textarea value={credsJson}
              ...
              rows={5} placeholder='{"claudeAiOauth":{"accessToken":"..."}}'
              spellCheck={false} autoCapitalize="off" autoCorrect="off"
              className="w-full px-3 py-2 rounded-lg text-[11.5px] bg-transparent outline-none font-mono"
              style={{ border: '1px solid var(--border)', color: 'var(--fg)' }} />
            <div className="text-[11px] mt-1" style={{ color: 'var(--muted)' }}>
              Файл не сохраняется у нас — сразу пишем на {deviceName} в ~/.claude/.credentials.json.
            </div>
