  ...
  return (
    <section className="bru-section">
      <div className="bru-container">
        <div className="bru-eyebrow">▶ FAQ</div>
        <h2>
          Honest answers
          <br />
          <span className="accent">to honest questions.</span>
        </h2>

        <div className="bru-faq-list">
          {QUESTIONS.map((item, i) => {
            const isOpen = open === i;
            return (
              <div key={item.q} className="bru-faq-item">
                <button
                  className="bru-faq-q"
                  type="button"
                ...
                >
                  <span>{item.q}</span>
                  <span className="bru-faq-icon">{isOpen ? '−' : '+'}</span>
                </button>
                {isOpen && <div className="bru-faq-a">{item.a}</div>}
              </div>
            );
          })}
        </div>
